...
export default class Viewer3DFirstPerson extends React.Component {
  ...
  componentDidMount() {
    ...
    let { controls, pointerlockChangeEvent, requestPointerLockEvent } =
    ...
    let yInitialPosition =
      this.planData.boundingBox.min.y +
      (this.planData.boundingBox.min.y - this.planData.boundingBox.max.y) / 2 +
      humanHeight;
    ...
    this.keyDownEvent = (event) => {
      let moveResult = firstPersonOnKeyDown(
        event,
        moveForward,
        moveLeft,
        moveBackward,
    ...
    this.keyUpEvent = (event) => {
      let moveResult = firstPersonOnKeyUp(
        event,
        moveForward,
        moveLeft,
        moveBackward,
