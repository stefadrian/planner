  ...
  _createClass(Project, null, [{
    ...
    value: function duplicate(state) {
      ...
      selectedHoles.forEach(function (holeID) {
        ...
        var _Hole$create = _export2.Hole.create(state, selectedLayer, duplicateElement.get("type"), duplicateElement.get("line"), width, duplicateElement.get("properties")),
            updatedState = _Hole$create.updatedState,
            hole = _Hole$create.hole;

        state = updatedState;
        state = _export2.Layer.select(state, selectedLayer).updatedState;
        state = _export2.Layer.selectElement(state, selectedLayer, hole.prototype, hole.id).updatedState;
      });
